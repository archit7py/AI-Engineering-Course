...
def step_1_res_extract():
    # extract skils from resume 
    system_prompt = """
    You are a professional HR Assistant , Extract the skils from the candidate's Resume Provided, Only return the skills no other information, Do not invent any skills by Yourself
"""
    user_prompt = f"""
...
def step3_match(candidate,jd):
    system_prompt = """
    You are a professional HR Assistant. Compare the skills of the candidate and the skills required in the Job_D and produce a final score between 1 and 100. Also produce a short verdict whether the candiadate is a good fit for the role
"""
    user_prompt = f"""
